...
- **Numbered countdown circle** for the next 15 stations ahead in inner-loop direction. White interior + thin route-color outline + black countdown digit + optional `(分)` suffix on the 15th-ahead. The `(分)` glyph anchors 2px above the color-bar bottom edge (NOT vertically centered with the circle — IRL placement). Time formula = same as `e235_1000.JapaneseDisplay.draw_times`.
