...
| `transfers_by_view` | Optional. Per-station map keyed by `"<line>_<direction>"` (e.g. `"JY_inner"`, `"JT_south"`). Value is an object with optional ops: `{"drop": [base-slug names...], "edit": {base-slug: replacement-slug-ref, ...}}`. Both ops match by base slug name — `drop: ["keihin_tohoku"]` drops both plain and any `keihin_tohoku.<variant>` references; `edit: {"keihin_tohoku": "keihin_tohoku.oimachi_kamata"}` replaces whatever `keihin_tohoku`-base entry the flat list has with the variant ref. Drop applied first, then edit. `add` op reserved for future. Empty / absent on most stations; populate as IRL evidence (LCD photo, audio listening) shows divergence from the flat-list default. Default-render with no override shows whatever's in flat — encode observed views first, accept untested views as best-effort. |
