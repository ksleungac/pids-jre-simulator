...
Accuracy today is earned through a stack of guards layered on the reader. The target is no misreads at all, which is a fair bar for a fixed font at a fixed size in a fixed position — real-world OCR handles handwriting and photographs. Rate measured at roughly 1 misread per 960 samples before the 2026-07-22 fixes. Four approaches, all measured that day:

- Register glyphs by centroid/moments instead of a threshold-derived bounding box. The box's own edges move when the threshold clips a column, so the registration frame itself shifts.
- Slice digit cells by known geometry instead of rediscovering the layout from column runs every frame. That deletes a failure class rather than guarding it.
- Fuse several frames before reading. The driver samples ~3 Hz from a 60 fps source and discards 19 of every 20.
- Separate empty-cell detection from the adaptive threshold, which sits pinned at its clamp ceiling on ~82% of bright-content frames.

Held here alongside them is the guard-side alternative that was never built: when a per-field read returns `None`, publish that field's last known-good value instead, so the band and the drive log stop flickering to blank on a transient miss. It is the opposite pole of this entry and it stays open on purpose — a reader that does not miss has nothing to hold. Building it means answering the three questions it was filed with:

- How long a value may be held before it reverts to `None`. A stale cruise speed held through a stop is worse than a blank.
- That it belongs in the display layer rather than the detector, which already carries `prev_badge` for its own crossing tests.
- That it must never resurrect a value the hardening layer deliberately gated away, such as a badge-rejected stopping offset.

*(was #91 parent + #92, #93, #94, #95, #70)*

### The HUD is found on any screen shape, not just the ones measured

Downscaling made resolution itself a non-issue — every capture shrinks into the one 1080p model, so a new size costs no templates and no calibration round. What is still derived rather than seen is *where the HUD sits*, fitted against a 16:9 viewport, so some shapes are refused instead of guessed. The two that a screenshot would settle are already scoped as [#122 "21:9 and non-multiple-of-16 widths — the two geometry cases still unmeasured"](https://github.com/ksleungac/pids-jre-simulator/issues/122); this entry is the *general* fix behind them, which is unscoped.

It replaces derivation with a user-drawn box. The user aligns a 50%-opaque rect to the real HUD; the box locks to the HUD's fixed aspect so the scale axis — the one that actually breaks alignment — disappears; and the live speed / distance / badge readout confirms it by eye. A shipped preset box auto-applies on common sizes, so the common case stays zero-config. Multi-monitor and windowed play fall out of it for free, since capture would follow the selected rect's own output.

Three things are unsettled: how far to take auto-proposing the box (a scaled default against landmark-matching the badge pentagon), whether a recognised resolution applies its preset silently or shows the screen pre-filled for confirmation, and how a box is keyed and persisted.

One input is stale before any of it. The alignment tolerance window — the stopping-offset cell breaks at ±3% scale, ±8 px shift, and is the canary because it holds the smallest digits — was measured before the 2026-07-2x read hardening widened it, so re-measure against the committed fixtures rather than designing to those numbers. Current behaviour and the full support table: `auto_input/README.md` § "What a profile means, and what gets interpolated".
...
The pipeline is built and verified against ~48 stations. Filling it out is blocked deliberately: adding stations only pays once the models that render them exist, so it waits on E233. Three parked threads:

- Populating `stations.json` beyond the current corpus, prioritising stations served by at least one LCD-equipped line, with `transfers_by_view` entered as raw observations and never derived.
- Declaring E233's own badge policy when that model lands. Its sub-series render some entries as colour squares rather than icons, and that belongs in its own `transfer_info.py` rather than a parameterised DSL.
- The English size trade-off, with the shinkansen row parked at 12 pt.
...
**Stage 1 is settled (2026-09-19):** a train bound for a station off its route list (Utsunomiya 1545E → 熱海, data ending at 東京) names that station as its terminus everywhere, and the drive ends at the last listed stop as a hand-off. Rule and surfaces: `docs/DISPLAY.md` § "Terminus (`dest_stop_idx`)". What stays open is the swap across the boundary itself, which the author links to the frame swap. E233-3000 has no LCD, so that behaviour is ours to design rather than to match.
...
Two levers, neither taken. Encoding runs 75–321 kbps at 48 kHz stereo and much of it is mono content in a stereo container; normalising to ~96–128 kbps mono would take the corpus down substantially again, a bigger lever than pooling was. It is lossy and irreversible, so it wants a deliberate decision rather than a sweep.

Separately, different stations often play the same melody (高尾 / 荻窪 / 西八王子 share one; ~57 MB of such duplication corpus-wide). The per-line pool cannot collapse them, because each file is the melody *plus* that station's own closing announcement, so no two files are actually identical. Collapsing them means splitting the two apart and changing how STA plays. *(was #119, #120)*
...
`plot_drive.py` does not render stopping accuracy today, and one thing has to be settled before it can. A perfect stop and a game-forced reset after an overrun both read as an identical green `0cm` — the same OCR read, a different game state — so counting them together would report an accuracy figure biased toward perfect, and biased invisibly. Three signals could separate them, each evaluated per arrival over a context window:

- Whether a black-screen / OCR-FAIL frame sits near the `MOVING→STOPPED` transition, since the reset uses a black-screen fast-forward.
- Whether the speed trace decelerates smoothly to zero or jumps to it.
- Whether distance crosses through zero or negative before the reset frame.

This is arrival-event analysis inside the plot tool, not an OCR-layer job. *(was #15)*
...
### What the project exposes publicly, decided deliberately rather than by default

Three surfaces, and they are not one question. The **repo** carries no commercial font software as of 2026-09-03 and no narrative memory: both moved to `private-pids-jre-simulator`, and what belongs there is settled in `conventions.md` § Data layout. The **release** still ships Helvetica Neue and Neue Frutiger World as real font files, because they are not baked and the app has to draw with something; that half is scoped as [#116](https://github.com/ksleungac/pids-jre-simulator/issues/116) and is the only one with a mechanism already named. The **third** surface has had no pass at all — everything gitignored but not private, of which `audio_src/` is the largest at 112 MB of source recordings and by-ear verdicts, and whose verdicts already fail to travel between machines (`critical_lessons.md § 11`).

What is unsettled is the standard rather than the mechanics. Moving a thing costs one command now. The question is which exposures are worth closing: a licensed font inside a public release is a different risk from a third-party recording inside a private repo, and neither has been weighed against the cost of a clone that cannot build itself. Deferred on 2026-09-03 rather than decided in passing.
...
- **Giving `pre_stops` a positive identity in the render path.** Closed 2026-08-30, with one re-open condition below. Every renderer decides a station's mark by asking whether it has announcements: no `pa` and no `pa_at_station` means "the train runs through here", so it draws a chevron. Three unrelated things share that signature — a genuine run-through station, a `pre_stop` (on the line, off this service), and a route's own origin, whose `pa` is empty because there is no announcement for the station you are already sitting in. Both train models make the same inference, and `e235_1000/lower_lcd.py:791` already carries a one-off patch for a single symptom of it (the origin's name colour), which is the tell that it is one gap and not three bugs. Visible today as a run-through chevron on 相模湖 in E233-0's 6-station view at Chūō's first stop — the author saw the render and judged it not worth carrying: *"i think let's just not care about this lol."* **Re-open only if `pre_stops` needs to carry more information than it does now** (author, 2026-08-30): a data-model change is the event that makes the positive flag free, and until then inferring it is cheaper than the alternative. `display_offset` already holds the fact (`di < display_offset`) at every call site, so the repair stays cheap whenever it is wanted.
