...
- [ ] **Transfer-info display — corpus 22/22 in-spec ✓ (post 2026-05-05).** GOLDEN row-grouping + N=2/N=3 structural rules + `rows` data override + Step 4 column-aware anchor row placement (with 1-vs-2 shinkansen split) all landed. Remaining: promote `render_transfer` from `preview_transfers.py` to `displays/train_models/e235_1000/`; within-row distribution polish for stations not yet helped (赤羽, 千葉, slack<0 fall-back cases); recalibrate Step-1 N=5 tier with more IRL refs. ([WIP_transfer_display.md](WIP_transfer_display.md))
