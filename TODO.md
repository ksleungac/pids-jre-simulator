...
- [ ] **Entry-point flow — toggle-ON + mid-transit click-jump alignment.** Click-jump re-anchor (Layer 2 → Layer 1) shipped 2026-05-30 for the *parked* case only. Still open: (a) mid-transit click-jump (Layer 3 driving at re-anchor time) needs the silent-advance alignment rule rather than the parked anchor; (b) toggle-ON entry-point (probe Layer 3 with consensus, align App). Both gated on the Layer 1 silent-advance mechanism — NOT a `jump_to_stop` variant. (`WIP_autodriver.md` § "Pending — Entry-point flow".)
