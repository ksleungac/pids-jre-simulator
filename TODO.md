...
- [ ] **Hook plot generator into the debug bar — "download report" button.** Add a small icon (📊 / save / download) in the right corner of the debug panel. Click → invoke `data_tools.plot_drive.render_html_report` against the current session's open JSONL → write `<jsonl_stem>.html` to project root → console-log the path. Clickable region wired the same way as a future Save-plot button (forward `MOUSEBUTTONDOWN` from `app.py` main loop to `auto_input.handle_panel_click`). Re-runnable mid-drive (the JSONL is partial-write-safe).
