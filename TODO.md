...
- [ ] **Hook STOPPED badge state into the lower LCD — step 2 only (OCR-driven).** Step 1 (display + state machine) shipped 2026-04-28: `AppState.at_station` is the manual STOPPING flag; lower LCD renders the pentagon at curr_stop's cell when set; full-route + 8-station both done (8-station is "quick version" — pixel-tuneable params block). What's still open: have AutoDriver flip `sim.state.at_station` based on `auto_input_status["badge"] == "STOPPED"` so the visual reflects OCR truth not just user PageDown. Status dict already publishes `badge` on each capture; hook is just reading it and toggling state. Watch for races with `_next_pa`'s state mutations.
...
- [ ] **Interactive lower LCD — click station icon to jump.** Hit-test on the lower LCD's station cells; clicking a station calls `app.state.curr_stop = <clicked_idx>` (with the cnt_pa / skip / departure_time housekeeping). Doubles as the **manual resync mechanism for OCR**: when the auto-driver's segment-start anchor and the simulator's `curr_stop` desync (route reload, manual jump, OCR misfire), the user clicks the right station and the auto-driver's existing mismatch-skip logic re-anchors on the next sample. Station hit-test rects available in both full-route and 8-station views — adapt to whichever is currently rendered. **MVP unblocked 2026-04-28** — STOPPING state shipped, so click-to-jump now lands in STOPPING@target naturally (`jump_to_stop` already does this); previous "blocked on STOPPING state" framing closed. Design state in [WIP_lower_lcd_click_to_jump.md](WIP_lower_lcd_click_to_jump.md). Remaining: hit-test rects in both views, MOUSEBUTTONDOWN dispatcher, OCR resync entry-point Q1/Q2.
