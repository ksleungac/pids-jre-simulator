...
- [ ] **Live-validate 1a (in-process integration).** 1a is feature-complete + compiles, but only 1b (`data_tools/capture_game.py`) has a live-tested drive end-to-end (Keihin-Tōhoku 727B Omiya→Kanda flawless on 2026-04-27 morning). 1a needs an equivalent real-drive confirmation: PAs auto-fire, debug panel renders, manual PageDown overrides, PASSING-badge segments suppress arrival, post-PASSING under-threshold case fires correctly. (`AUTO_INPUT.md` § "Future enhancements" + § "For next-session Claude".)
...
- [ ] **Drive recorder / blackbox — plot generator.** Data-capture side **DONE**: AutoDriver streams every OCR cycle to `_recordings/drive_<line>_<diagram>_<TS>.jsonl` (locked schema in `auto_input.py`). Pending: a plot script that reads the JSONL and renders an interactive HTML (Plotly) — speed line + fill (D2), real-time X axis (B1), gray bands over STOPPED zones (C1), single continuous timeline (no row-break decision), hover tooltips with full OCR fields. Inline plotly.js for portability. Output `drive_<...>.html` at project root. Generation triggered by a "Save plot" button on the debug bar (forward MOUSEBUTTONDOWN from app.py main loop into `auto_input.handle_panel_click`). Plotly to be added as dev dep when this lands.
