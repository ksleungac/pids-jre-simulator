...
- [ ] **Drive recorder / blackbox** (cool feature). Persist every OCR sample (timestamp, speed, distance, badge, fired-flags, current stop) to a per-run log file (`audio_src/_recordings/<line>_<diagram>_<YYYYMMDD-HHMM>.jsonl` or similar — gitignored). Post-run, plot speed-over-time / distance-over-time / position graphs (matplotlib script in `data_tools/`). Useful for: visualizing how the user actually drives a route, debugging missed-fire moments after the fact, comparing runs. Cheap to add since the AutoDriver already has the data — just append to a file each cycle. Decisions deferred: per-run vs continuous file, retention policy, plot tool location.
