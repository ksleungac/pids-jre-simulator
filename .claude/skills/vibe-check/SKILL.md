...
Multiple AI sessions add features cleanly enough on their own, but each session has a limited view of the whole. Over time the codebase accumulates:

- duplicated helpers — someone forked `utils.py` instead of editing the original;
- dead abstractions — a registry or factory created for "the future" that never came;
- unused imports left from a different prototype;
- stale comments describing a structure that has been refactored away;
- two ways to do the same thing within one class.
...
1. **Duplicated logic / forked helpers.** The same function defined in two files, or near-identical blocks copy-pasted across classes. Especially: when a helper got "evolved" (added a new parameter, new branch) by forking the file rather than editing in place. The fork is the vibe-coding signature.
...
11. **Local utility helper that should live in a shared module.** A small utility function (path resolver, JSON loader, slug parser, format helper, common regex, anything stdlib-only and < 20 lines) lives in a feature module instead of a shared utils home (`app_paths.py`, `displays/utils.py`, `constants.py`, `i18n.py`, package-local `utils.py`). Often duplicates similar helpers in other files. **Trigger heuristics:** function < 20 lines, body uses only stdlib (`pathlib`, `json`, `re`, `sys`, `os`), name like `load_*` / `resolve_*` / `parse_*` / `format_*` / `_*_root` / `_*_path` / `_*_dir`, no domain-specific imports. **Verification:** grep for similar function names cross-codebase. If multiple implementations exist → fires #1 (forked helpers) too. **Fix:** promote to a shared module + import from there. If genuinely single-feature-specific, leave but document with a one-line comment. Per `principles.md` § "Search before authoring a function, a script, or an instrument" — authoring locality is the failure mode this catches.
12. **Deployment-frame / runtime-semantics primitive outside canonical home, without verification anchor.** Three shapes qualify: a reference to `sys._MEIPASS`, `sys.frozen`, `sys.executable`, `Path(__file__)` for behavior-dependent paths, or branching on `if frozen:` / `if dev:`; threading primitives in non-obvious patterns (locks held across yields, queue draining at sentinel times); file-I/O timing assumptions (write-then-read, mtime checks). It is a finding when that code lives outside the canonical home (`app_paths.py` for path resolution) AND has no inline `# CONTRACT:` block citing primary source.

    **Distinct from #11**: #11 catches duplicated *helpers*, #12 catches *code that depends on external behavior* — runtime conditions not visible from reading the file. The 2026-05-05 release-crash incident had four such sites that read as plausible locally and were wrong because the deployment-frame primitive was misapplied. **Verification:** for path resolution, confirm centralization in `app_paths.py`; for threading and I/O, confirm a comment explains the invariant. **Fix:** promote to canonical home, or add a CONTRACT block citing primary source — the build script for PyInstaller behavior, the library hook for runtime quirks, OS docs for OS-specifics. Per `principles.md` § "Verify runtime semantics from primary source"; defending these from memory is the failure mode this catches.
13. **Integration residue — redundant-but-wired screen / branch after a flow integration.** When a flow becomes default, or a screen's job moves to a sibling feature, the now-redundant first-run screen or state-gated branch is often left reachable. It is invisible in dev because it only fires on absent persisted state, and a populated `settings.json` masks it. Per `critical_lessons.md §6`. **Verification:** for each first-run / onboarding / OOBE screen, confirm a live entry path AND that no sibling already owns its function. The v0.6.0 language picker survived redundant beside the TIMS home's language knobs for the whole graduation arc. **Distinct from #2** (dead/unreachable): this code IS still reachable, just no longer *needed*.
