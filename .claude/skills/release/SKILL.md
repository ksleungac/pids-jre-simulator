...
**Read the commits CAREFULLY — reconstruct the timeline, don't skim.** A large release (50+ commits) is not licence to judge from subjects at a glance. Read the messages, and the bodies where they matter, to build the shape of the release: which commits are iterations of ONE body of work (many `feat(setup)` commits = one first-release feature, not many); what is a genuine fix to a PRE-EXISTING shipped feature against polish on something new this release; and what a later commit superseded. That timeline judgment is what sets correct weighting and categorization. Skimming is what produces a one-line blurb for a huge feature and a "Fixes" bucket full of a new feature's internals. Per `principles.md § "Verify before claiming"` ("deriving release/history facts: read the commit messages in range").

**A commit's SUBJECT hides user-facing fixes; the body is where they are.** Sweep the range a SECOND time asking only "does any body contain a fix to a PRE-EXISTING shipped feature". The first pass classifies by subject, and a subject like `fix(display+data): review-finding pass` or `fix(display): band fill fires on slot-enter` reads as internal housekeeping. 2026-08-11: two real ones surfaced that way after the notes were already drafted — a five-station fill animation that restarted whenever the window moved or lost focus, and a lower display that locked to the zoomed view near a route's end. Neither was inferable from its subject.

**Name each fix by the COMPONENT it belongs to, not the route it was found on.** Both of those are E235-0 fixes, and that model accepts any route, so filing them under "Yamanote" would have been wrong for every other line a user drives it on. (`conventions.md § Naming`, cause not discovery instance.)
...
- Announce what is NOT built yet, unless leaving it out would mislead a reader into expecting it or it is critical. A gap is not a feature (author, 2026-09-19, striking "English mode is not built yet" from v0.7.0's E233-0 entry).
- Write prose the screenshots already show. When a gallery shows the views, the text does not list them.
