...
- **Deployment-frame and runtime-semantics verification.** Verify against primary source whenever the diff touches PyInstaller path semantics (`sys.frozen`, `sys._MEIPASS`, `sys.executable`, `Path(__file__)` for behavior-dependent paths), threading primitives, file-I/O timing assumptions, library behavior that differs between dev and frozen, or any code whose correctness depends on runtime conditions not visible from reading the file. Primary source means the build script's actual copy logic, the library's runtime-hook source, official docs, or exercising the deployed artifact. Don't accept "intentional" or "standard" claims from memory. In the 2026-05-05 release-crash trace a prior reviewer defended `i18n.app_root`'s `_MEIPASS` usage as intentional semantics, reasoning from generic PyInstaller lore rather than this project's build script. That script doesn't use `--add-data`, so `_MEIPASS` is empty. Per `principles.md` § "Verify runtime semantics from primary source".
...
11. **Local utility helper that should live in a shared module** (path resolver / JSON loader / slug parser / format helper / common regex / stdlib-only helper buried in a feature module instead of `app_paths.py` / `displays/utils.py` / `constants.py` / `i18n.py`). Per `principles.md` § "Search before authoring a function, a script, or an instrument".
12. **Deployment-frame / runtime-semantics primitive outside canonical home, without verification anchor** (`sys._MEIPASS`, `sys.frozen`, `sys.executable`, `Path(__file__)` for behavior-dependent paths, `if frozen:` branching outside `app_paths.py`; OR threading / I/O timing patterns without a comment explaining the invariant). Per `principles.md` § "Verify runtime semantics from primary source". Sibling to Lens 1 verification — Lens 1 fires on the diff; Lens 2 fires on the standing pattern.
13. **Integration residue: a screen, branch or flag made redundant by a sibling feature but still wired.** After a flow integration, where a flow becomes the default or a screen's job moves to a sibling, the now-redundant first-run screen or state-gated branch is often left reachable. It is invisible in dev, because the branch only fires on absent persisted state. **FULL and INTEGRATION scope only**, since a DIRTY diff is structurally blind to it: the stale branch is unchanged history. This is not #2 (dead code) — the code is still reachable, just no longer needed. Per `critical_lessons.md §6`. **Verification:** for each first-run or onboarding screen, confirm a live entry path, and confirm no sibling feature already owns its job. The v0.6.0 language picker was redundant beside the TIMS home's language knobs.
