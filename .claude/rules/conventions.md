...
- **`_*` prefix = preserved-but-not-shipped staging.** Examples: `audio/_archive/` (recordings the active diagram doesn't use, kept for future glob lookup), `audio/_mock/` (preview/test fixtures), `_dev_scripts/` (middlescript: audio splitters/validators, OCR-asset extractor, primitive-bans linter, dxcam + game-capture diagnostics), `_experiments/` (WIP not yet ready for project root), `_<feature>_calibration/` (gitignored source material that derives committed artifacts).
...
- **Structural renames: pre-commit grep of the old name.** After any multi-file rename or package restructure, run `grep -rn "<old-name>" --include="*.{py,md}"` before committing. Third-man catches high-traffic sync targets; tail-of-distribution stale refs (rationale paragraphs, dev-script comments, table parentheticals) only surface under a literal grep. Historical entries in `critical_lessons.md` / `memory/*.md` are explicit don't-touch.
