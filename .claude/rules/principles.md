...
Recurred 2026-05-07 on the OS-specifics axis: a real user reported instant-launch crash; claude scanned the repo for SysFont usage, found `i18n.font_named()`, and dismissed it as "by design / not a bug" reading only the i18n.py local docstring's "Windows ships YaHei… acceptable" framing. The CONTRACT block at `displays/train_models/*/upper_lcd.py:144` — saying *"never `pygame.font.SysFont()` — SysFont scans the Windows font registry, which fails on Chinese/Japanese"* — was IN THE SAME GREP OUTPUT and went unread. The 2026-03-14 memory entry documenting the exact `TypeError: expected str, bytes or os.PathLike object, not int` crash mode was one grep away. Same pathology: cached/local docstring framing accepted as authoritative; canonical rule corpus (CONTRACT block + memory incident) sat in or one step from context, unconsulted. User: *"yes but why did you answer me no where uses sysfont?"* — the load-bearing framing ("by design / not a bug") was the failure shape this principle warned against.
...
Recurred 2026-05-05 in a vocabulary-shifted shape (same root, no primitive replaced): claude proposed within-scope changes to Step 4 row 0 placement but framed them as a NEW "column model" abstraction with Phase 1/Phase 2 staging, options A/B/C, and separate-step pipeline diagrams. The actual change was just modifying one branch of Step 4. Took ~5 user redirects (*"use simple english"*, *"don't be so complicated"*, *"as per golden algorithm, the current steps. use that as a reference, otherwise I will lose focus"*, *"How does this fits into the current steps modelling"*) before claude grounded the proposal in the existing GOLDEN Step 1-4 vocabulary. The framing inflation was the friction source even though scope was right.
...
- **Frame within-scope changes in the user's vocabulary at the user's granularity.** Even when the change is genuinely inside the framework, presenting it as a NEW abstraction (new model, new concept, new pipeline step, Phase 1/2 staging) creates the same focus-loss as actually replacing a primitive — the user has to translate claude's framing back to their own framework before they can engage. Match the user's granularity: "modify Step 4 row 0 placement" beats "introduce a column-aware lookahead model"; "enhance Rule 2's tail-anchor selection" beats "add a Phase 2 anchor-optimization pass". The vocabulary used in the proposal IS the proposal's scope to the user, regardless of what the underlying code change actually touches.
