...
- (2026-05-14) Designed calibration-editor clock tuneable shape with bg rect as a separate sub-element from CLOCK_RECT, treating the inset `pygame.Rect(clock_x, 5, 80, 25)` bg fill as a tuneable thing. User: *"sound like some bloat here? ah but because it crashes with my blip zone idea, should be in DISPLAY.md."* DISPLAY_E235.md § "Element confinement" line 66 explicitly says "the clip wrap, the bg fill, and the debug-grid tint all read from the same constant" — every other region honors this; only draw_clock had a sub-sized bg with hand-tuned magic numbers. Should have re-read DISPLAY_E235.md before designing a doc-governed pattern, not after user pointed me at it.
...
- **Structural renames also need a pre-commit `grep` of the old name, even with third-man.** Third-man's primary-source read catches high-traffic sync targets (build script, top-level docs, primary headings). Tail-of-distribution stale refs — rationale paragraphs, mirror comments inside dev scripts, table inline parentheticals, JSONL row descriptions — escape third-man's pass and the original commit's sweep both, and only surface under a literal `grep -rn "<old-name>" --include="*.{py,md}"` across the repo. Run that sweep before the commit; every hit is a sync candidate (or, for `critical_lessons.md` / `memory/*.md` historical entries, an explicit don't-touch). Evidence: 2026-05-14 OCR package refactor — third-man caught 3 sync items pre-write; /review+fix surfaced 6 more dangling stale `auto_input.py` refs post-commit.
