...
262×360; readers then run against `DOWNSCALE_PROFILE`. This is the default for every
resolution. `AutoDriver.legacy_ocr` (`main.py --legacy-ocr`) forces the old per-resolution
path instead — a debug lever for isolating a downscale-side fault, not a supported mode.
...
Both resolutions driven through `ocr_observe`, which runs the production read path.

| | 1440p (ratio 0.75) | 4K (ratio 0.50) |
|---|---|---|
| live samples | 792 over 453 s | 941 over 566 s |
| badge / speed / distance / limit read | 100 / 100 / 98.4 / 100 % | 99.1 / 99.3 / 98.2 / 99.4 % |
| tenths | 99.5 % | 99.3 % |
| speed_score median | 0.880 | **0.883** |
| dist_score median | 0.875 | 0.869 |
| badge_diff median (reject at 50) | 9.8 | 10.1 |
| implausible distance jumps | 0 | 0 |
...
- **Non-16:9 is untested.** 16:10 / 21:9 are refused rather than guessed, because nobody has checked whether the HUD keeps its screen fractions when the aspect changes. Needs one screenshot at such a resolution, not a design decision.
...
- Whether `--legacy-ocr` is worth keeping at all once the live drive confirms the downscale path, or whether the per-resolution machinery retires entirely.
...
- **Distance plausibility guard** (`guard_distance`, `driver.py`, 2026-07-21) — rejects a distance read that moves further than `MAX_DRIVABLE_SPEED_KMH · Δt` from the last valid one and holds-last-good; gated to steady MOVING so a legitimate target switch re-anchors. Wild jumps 24/10 → 1/1 on the reporter's 1080p logs.
