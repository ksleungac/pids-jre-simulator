...
| **T2 Contract** | schema / parity | authored *data* conforms + cross-file parity | ✅ EXISTS | `validate_data.py` (root), `_dev_scripts/validate_ocr.py` |
| **T3 Invariant** | integration, headless | cross-module *behavior* over real files, **no display** | ⬜ GAP | `_tests/t3_invariant/` |
...
2. `✅ T1` **`test_detector_update_seq.py`** — `_Detector.update()`, the DIRECT read (raw sample → events/flags). Cases: departure crossing + no-refire debounce · arrival level-test · **PASSING skips arrival** · `STOPPED→MOVING` segment reset · black-screen cross-reject · departure-through-PASSING. **Highest-value T1;** stateful (call sequence). *Done 2026-07-16.*
3. `✅ T1` **`test_reentry_target.py`** — `AutoDriver._resolve_reentry_target()`, the bug site. 18-cell grid + regression `PASSING @ speed<30, parked → None` + facet `resolve(PASSING,s,d) == resolve(MOVING,s,None)` + never `1B` + guards. SimpleNamespace sim stub. *Done 2026-07-16.*
...
- OCR pixel→symbol reads — owned by T2 `validate_ocr.py` (exists); confirm it includes a PASSING + low-speed frame.
