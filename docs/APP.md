...
- **A failed read says `?`, never `--`.** `--` is a well-formed answer meaning "nothing is posted here", true for a whole drive on a line with no speed limit, so wearing it for a failure makes the failure look normal. The unit stays either way. Dim `?` means the row stood down because the badge failed; amber `?` means it failed on its own; an amber number was accepted under the score bar. Full table and rationale: `auto_input/README.md § "Fault marks"`.
