...
Track geometry lives in `_TUNEABLES_FULL_ROUTE` (`e235_0/lower_lcd.py`, calibration-editor `full_route` element — press `V` to reach the full-route view). `curve_v_radius` is derived from the two row centerlines rather than stored, so the cap radius cannot disagree with them.
...
**Major-station bold names**: `MAJOR_STATION_NAMES_BOLD = {上野, 東京, 品川, 新宿, 渋谷, 池袋}` — module-level hardcoded set, **not** derived from `code_3` (大崎/JY24 has code_3 = OSK but is NOT in the bold set per IRL). Names in the set render with `ShinGoPr6N-Heavy.otf`; the rest use `ShinGoPr6N-Medium.otf`. Both at 19pt.
...
1. **Per-N text scaling** — JA size from `N_total` + shinkansen-count: Sparse (≤4) 32 / N=5 29 / Mid (6-9) 26 / Dense (≥10 OR ≥2 shinkansen) 22. EN derived `round(JA × 12/23)`. `name_line_gap` scales with EN. "Both shinkansen" (Tokyo JO) forces Dense regardless of N. The four sizes are one list (`name_size_ladder`) so shrink-to-fit can step down it.

   **Shrink-to-fit.** The ladder keys on ENTRY COUNT, which under-predicts width when a line name is long, so a low-N station can still overflow its row. `Last-ditch (pack from margin)` is the placement pipeline reporting exactly that — it fires when no rule could place the row, collapsing every gap to 0 and sometimes running off the canvas. On that signal the renderer steps one rung down the ladder and lays the whole panel out again (`size_step`, bounded by the ladder). The background fill and banner have already been drawn, but the retry's own `surf.fill` wipes the surface and redraws them identically (banner sizes do not depend on `size_step`), so nothing partial survives; every measurement is re-derived at the new size, making it a genuine re-run rather than a patch-up. `_size_tier` rises by exactly 1 per call, so recursion depth is at most `len(name_size_ladder) - 1`. It cannot reach a panel that placed cleanly. Corpus effect: 2 panels, both previously degenerate — 秋葉原 `JY_inner` (N=3 → Sparse 32pt, but 総武線各駅停車 + つくばエクスプレス線 measure 620px against 618px of usable row, so the pair rendered touching; 29pt clears it) and 上野 `JK_south` (26pt → 22pt, which also retires its −38px canvas overflow). After it, no panel in the corpus reaches Last-ditch and none has a negative gap.

   *Known gap:* Rule 1's intrusion check passes on `predecessor_right ≤ upper_anchor`, so an entry may anchor flush against its predecessor's right edge — a legal placement with a **zero** gap that never reaches Last-ditch and so is invisible to shrink-to-fit. One instance: 大宮 `JA_north` row 2 (`inter=[0, 40]`), where 埼京線's column x equals 京浜東北線's right edge.
...
- Last-ditch `pack-from-margin` fires on 2 corpus panels (秋葉原 `JY_inner`, 上野 `JK_south`) and is now the trigger shrink-to-fit keys on — see § Pipeline step 1. It never survives to a rendered frame: the retry re-lays a rung smaller and no rendered panel reaches it.
