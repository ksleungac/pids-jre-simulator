...
**The committed badge fixtures are not a gate.** All six `_tests/fixtures/ocr/1080p/cells/badge__*.png` are byte-identical to the six anchors they are matched against, so `classify_badge_state` scores them 0.00 and the T3 badge assertions are the artifact compared to itself — [critical_lessons.md §10](../.claude/rules/critical_lessons.md) recorded this on 2026-08-19 and it is still true. Re-cutting them from a capture the anchors did not come from is what would make T3 able to see a classifier change at all; until then the badge's real coverage is T1 `test_ocr_read.py` § 5, which reasons about transforms rather than pixels.
