...
**Threshold is per-cell, not a constant** (`_cell_dark_threshold`, 2026-07-22). Otsu over the text band, falling back to `DARK_THRESHOLD` when the band's dynamic range is under `OTSU_MIN_CONTRAST`.

**The clamp on that threshold is ABSOLUTE (`[OTSU_CLAMP_LO, OTSU_CLAMP_HI]` = `[55, 100]`) and that is a known weakness.** On 15 of the 23 committed cells the true Otsu already sits above 100, so the clamp binds on most normal reads and the adaptive threshold is in practice pinned at its ceiling; under a lifted capture it pins there from about +30 on. #89's silent digit drops under brightening (`3417 → 34`) are its consequence.

A relative clamp — fractions 0.30/0.70 of the cell's own range — was built and **reverted on 2026-09-09**, and the reversal is the more useful record. It looked strictly better on every proxy: identical reads on all 23 committed cells, survival to a +100 lift where the absolute clamp starts losing cells at +60, and it refused a blocky-scenery cell the absolute one hallucinates `speed=30` out of. Replayed over the 6838-frame live corpus it was a net regression — **341 of 1962 1440p frames lost their speed decimal** (`9 → None`), with wrong distances (`3243 → 3253`, `26 → 46`) and dropped reads alongside. The fixtures and the synthetic ramp were the proxy; the corpus is the outcome, and they disagreed (`principles.md` § "Validate against the outcome, not a proxy"). What it wants is the ordering the badge classifier uses: absolute pass first and unchanged, relative threshold reached only on a cell the first pass read nothing from, so non-degradation is structural rather than measured. Tracked in [#89](https://github.com/ksleungac/pids-jre-simulator/issues/89).
...
- PASSING's blue fill widens the margin — cross-state mean diff to MOVING/STOPPED ≈ 43, against within-state ~6–10 — but it is **not what the classifier lives on**. Desaturate the anchors completely and all six still self-classify, at a diff of only 25. The **text content** carries the discrimination; the colour helps. Measured 2026-09-08 (`_dev_scripts/_badge_colour_shift.py`), and it matters because it means a capture that shifts colour is survivable while one that shifts LEVELS is not.
...
Diff < 15 = high confidence; real reads sit there. Diff > `BADGE_DIFF_REJECT` (50) is rejected by the raw pass.

**Above the reject the metric has genuinely stopped discriminating — it is not a good read being discarded.** Measured over 7 subjects under a global lift: at diff ~54 raw still ranks the right anchor 7/7 and only the threshold refuses it, but at ~75 it ranks right just **3/7**, because a lift large enough to clip the badge's bright fill collapses the three states toward each other. So raising `BADGE_DIFF_REJECT` is not the fix for a level-shifted capture: it would return a state that is wrong about half the time, and the driver treats a wrong badge as fact while it treats `None` as "keep `prev_badge`". The shape-only pass ranks 7/7 at every shift measured, which is the reason it is a metric change rather than a threshold change. Black-screen / dark-cell garbage frames diff 60-110; mid-animation transient spikes at exact transitions hit >70. Detector's `None` tolerance preserves `prev_badge` across rejected frames, so one-cycle delay on transition detection = only cost.

**Two passes, raw first.** `classify_badge_state` returns `(state, diff, via, level)` — `level` is the capture-shift figure documented three paragraphs down. The raw pass runs first and unchanged, so any frame that classifies today classifies identically — non-degradation is a property of the ORDER rather than a measurement, which matters because the committed badge fixtures cannot prove it (below). Only once raw has rejected does a second, **shape-only** pass run: `1 - pearson r` against the same anchors, rejected above `BADGE_NCC_REJECT` (50). `diff` is always the RAW number whichever pass answered, so an old drive log's `badge_diff` still means the same thing.

The second pass exists for a capture whose **levels** are shifted. #137's reporter reads badge at 6–45 % with a diff median of 70–80 while speed and distance on the same frames read 87–99 % at normal scores — the digit readers binarise to shape, the badge did not. Measured against the anchors under 34 global transforms, the worst real badge scores 26.5 on the shape-only metric while black / white / uniform / noise / gradient all score 97–100, so 50 sits in a wide empty band rather than being fitted. It is not a loosening: on every garbage input measured it is **stricter** than raw, which accepts a flat mid-grey cell at 44.2 where the shape metric scores it 100. That raw weakness is pre-existing, pinned as current behaviour in T1 `test_ocr_read.py`, and deliberately not fixed here — fixing it would change frames production currently classifies. `badge_via` (`"raw"` / `"ncc"` / `null`) is published per sample so the second pass's engagement rate is visible in the drive log instead of silent. See [#143](https://github.com/ksleungac/pids-jre-simulator/issues/143).

**The shape-only metric correlates LUMINANCE, not RGB.** Ravelling the three channels into one vector cancels `a·v + b` only when `a` is common to all three, so a per-channel colour cast does not cancel — and both an HDR tonemap and a ReShade preset apply one on top of their curve. A heavy cool cast turned the green MOVING/STOPPED cells into blue ones that correlated better with PASSING, 2 of 6 correct. Discarding colour is affordable because the discrimination is not there: fully desaturated anchors still self-classify on the raw pass, so the badge text carries it. Measured over 74 rungs across 10 axes — lift to +220, gain to ×6, crush to ×0.04, gamma to 0.08, full desaturation, warm/cool/green casts, HDR paper-white to 6400 nits, ReShade to 6× strong — the luminance form is **74/74 correct**, worst real 39.8, best garbage 96.4. Per-channel correlation was tried first and is worse: a channel that saturates flat yields no correlation at all, so it scored 100 on real badges and left no gap above garbage. `screenshot_badge_shift_ladder.png` (regenerated by `_dev_scripts/_badge_colour_shift.py`) is what each rung looks like.

**`badge_level` is published per sample too** — the cell's mean minus its nearest anchor's, i.e. how far this capture's levels sit from the ones the anchors were cut at. It is computed from the raw pass's winner, so it exists even on frames both passes refuse, which is when it is most wanted. Nothing branches on it. #137's ~+90 had to be inferred from `badge_diff` medians across ten attached logs over several weeks; with this field a report arrives carrying its own answer ([#89](https://github.com/ksleungac/pids-jre-simulator/issues/89)).

**It has been seen rescuing real frames, but not the ones it was built for.** Replaying #137's reporter's screen recording through `ocr_replay_video.py` reads badge on 782 of 782 samples, 61 of them via the shape-only pass. Those 61 fall into six runs of 0.4–6.4 s in a 72.8–76.4 band, and they are all the **pause menu**: the ESC dim, not a level lift. Their baseline outside the menu reads 27.2–27.5, a healthy number, and our own frames through the identical 1080p → 720p → x264 → 1080p chain cost 15.8–22.0, so nearly all of it is the transcode. YouTube serves the video at 1280×720 BT.709 SDR with no HDR variant at any resolution, so the tonemap removed the lift the replay was meant to find. A screen recording cannot answer a levels question; `#140` said so before the fact and it was right.

**The pause dim is a large downward level move**, which is what makes it worth measuring rather than a nuisance. The badge cell's mean goes 134.0 → 65.9 and 134.6 → 65.8 across two independent runs, about ×0.49, and it costs a correct capture +49 of diff. That is the mechanism behind the reporter's *"when I enter the pause menu, OCR will be able to auto advance"* — the same dim that breaks a correct capture rescues a lifted one.

**What produces #137's lift is still unidentified.** Modelled in linear light, an HDR desktop at 400-nit paper-white costs only 42.7–47.8, inside the reject, against their ~+90, so HDR alone does not explain it. The unmeasured step is what dxcam returns when DDA duplicates an FP16 scRGB surface and `_open_capture_camera` asks for `output_color="BGRA"` — [#140 § "Unverified"](https://github.com/ksleungac/pids-jre-simulator/issues/140). One `ocr_observe.py` run on an affected machine is what settles it; no ReShade capture has been read at all.

**Seven badge fixtures, and six of them are not a gate.** `badge__running_en`, `..._ja`, `stopping_*`, `passing_*` are byte-identical to the six anchors they are matched against, so `classify_badge_state` scores them 0.00 and those T3 assertions are the artifact compared to itself — [critical_lessons.md §10](../.claude/rules/critical_lessons.md) recorded this on 2026-08-19. `badge__reporter_slip_lowspeed.png` is the exception, added 2026-09-09: cut from a reporter's capture, so its pixels come from another machine and it scores **11.5**, which is what a real read looks like. It is the whole of T3's honest badge coverage, and it is also the extra subject T1 § 5 puts every level shift through, so a shifted cell starts at the ~12 of ordinary mismatch rather than a shifted anchor's 0.

Everything in `_ocr_calibration_1080p/` other than that one file is a near-duplicate of the anchors' own session: the badge cell's mean level sits within ±2 of them on all 20 remaining frames, and two are byte-identical. So this corpus holds **no** capture with #137's signature, which is why the shifts stay synthetic. `_dev_scripts/_badge_fixture_sources.py` is the survey, and it writes a contact sheet if the labels need re-checking by eye.
...
**Derivation is pure and lives in `sampling.fault_states`**; the driver adds hysteresis (`FAULT_RAISE_SAMPLES` = 2 agreeing samples to raise, `FAULT_HOLD_S` = 1.5 s to clear) because the band renders at frame rate while a sample arrives once per interval, so the renderer cannot count consecutive ones. At the 3 s default that is 6 s of persistent fault before a mark shows, which is also why `#110`'s ~1 s m/cm transition at arrival cannot raise one: replaying all 6,838 corpus samples at their denser 0.55 s capture cadence still produced a single visible episode in nine transitions. The hold only extends a mark at the 1 s setting. Published as the `faults` status key. Display-only: nothing here feeds a fire decision, so a wrong mark costs a wrong-looking band and never a wrong announcement. Locked in T1 `test_auto_driver.py` § 6; preview with `uv run _dev_scripts/preview_band_ocr.py --faults`.
